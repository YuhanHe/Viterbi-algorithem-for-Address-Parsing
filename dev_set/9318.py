...
def read_state_file(file_name):
    ...
    for i in range(number_of_state):
        for j in range(number_of_state):
            A_ij[i][j] = float((N_ij[i][j] + 0.0001) / (number_of_state + 0.0001*N_i[i] - 1))
    for i in range(number_of_state):
        A_ij[i][-2] = 0
        A_ij[-1][i] = 0
...
def read_symbol_file(state_file_name, symbol_file_name):
    ...
    for i in range(number_of_state):
        for j in range(number_of_symbol):
            B_ij[i][j] = float((N_ij[i][j] + 0.0001) / (number_of_symbol + 0.0001*N_i[i] + 1))
    for i in range(number_of_state):
        B_ij[i][number_of_symbol] = float(0.0001 / (number_of_symbol + 0.0001*N_i[i] + 1))
...
r = viterbi_algorithm('State_File', 'Symbol_File',
                      'Query_File')
for ele in r:
    print(ele)
